...
- (void)processImage:(cv::Mat &)image {
    cv::Mat img, gray, bw, hsv;
    cv::cvtColor(image, img, CV_RGBA2RGB);
    cv::cvtColor(img, hsv, CV_BGR2HSV);
    cv::inRange(hsv, cv::Scalar(10, 60, 200), cv::Scalar(30, 255, 255), bw);
    ...
    for( int i = 0; i< rects.size(); i++ ){
        ...
        if( w < 50 && h < 50 && h > 10 && 1.1 < hw &&  hw < 5) {
            ...
            if(index != 10){
                char tmp[10];
                sprintf(tmp, "%d", index);
                cv::putText(image, tmp, cv::Point(x, y), cv::FONT_HERSHEY_DUPLEX, 0.5, cv::Scalar(0, 255, 0, 255));
            }
        }
    }
//    cv::cvtColor(bw, img, CV_GRAY2BGR);
//    image = img;
}
